...
if ($data) {
	...
	for ($i = 0; $i < count($data['ct']); $i++) {
		...
		$instTitle = bpoc::generateLongName($data['ct'][$i]['nl1']); 
		$featuredTitle = $data['ct'][$i]['n'];
		$link_featured = '/objectview/'.$sAction.'/'.$data['ct'][$i]['pk'].'/'.urlencode(str_replace(' ','-', $featuredTitle));
		
		if ($data['ct'][$i]['l_desc'] ==''){
		$museum = CollectionsController::museum(urldecode($data['ct'][$i]['nl1']));
		...
		$pdf->core->Image($image, 44.7, 6.6, 0, 115, 'JPG', BPOC_HOST.$link_featured, '', true, 72, '', false, false, 0, false, false, false);
		...
		$pdf->core->MultiCell(281.7, 0, $featuredTitle, 0, 'L', 1, 0, 5, 137.8, true, 0, false, true, 0);
		$pdf -> core -> SetFont('helvetica', '', 10);
	...
	$pdf -> core -> lastPage();
	$pdf -> core -> Output('balboa-park-commons-featured-collections.pdf', 'I');	
	}//end if
