...
if ($data['ct']) {
	$title = $data['ct'][0]['pk'];
	CakeLog::write('debug', 'in pdf_item for PK' . $title);
	...
	for ($i = 0; $i < count($data['ct']); $i++) {
		$item_link = BPOC_HOST . '/objectview/item/' . urlencode($data['ct'][0]['pk']) . '/' . urlencode(str_replace(' ', '-', $data['n']));
		//start adding pages
		...
		$pdf->core->Image(P_HOST.$data['ct'][0]['w'], 44.7, 6.6, 0, 115, 'JPG', $item_link, '', true, 72, '', false, false, 0, false, false, false);
		...
		for ($d = 0; $d < count($data['ct'][0]['md']); $d++) {
			...
			//never show these
			if ($data['ct'][0]['md'][$d]['mt'] == 'PUBLIC COMMONS.INSTITUTION DESC' || $data['ct'][0]['md'][$d]['mt'] == 'PUBLIC COMMONS.COLLECTION COVER IMAGE' || $data['ct'][0]['md'][$d]['mt'] == 'PUBLIC COMMONS.FILTER') {
				$metaHtml .= '';
				$left_column .='';
				$right_column .='';
			}  	
			
			elseif ($data['ct'][0]['md'][$d]['d'] && $data['ct'][0]['md'][$d]['mv'] ==''){
				$left_column .='';
				$right_column .='';	
			}
			
				else{
						$wLenght = strlen($data['ct'][0]['md'][$d]['mv']);
						$cutOff  = 140;
						$loopIt = ceil($wLenght / $cutOff);
					 
						if ($wLenght > $cutOff ){
						$left_column .= $data['ct'][0]['md'][$d]['d'];	
						$clean = strip_tags($data['ct'][0]['md'][$d]['mv']);
						...
						$left_column .= $data['ct'][0]['md'][$d]['d']."\n";
								...
								$right_column .= strip_tags($data['ct'][0]['md'][$d]['mv']). "\n";
								}
	...
	//$pdf -> core -> lastPage();
	$pdf -> core -> Output('balboa-park-commons-image-' . $data['ct'][0]['pk'] . '.pdf', 'I');	
	}//end if
